...
1. 答案必须正确且唯一
2. 只输出纯JSON，不加其他内容
3. 【重要】解析步骤中的每一步计算必须正确，禁止出现算术错误
4. 【重要】分数运算时，3/8表示八分之三（0.375），不是3除以8等于9
5. 生成题目后，必须验算答案和解析中的每一步计算
...
const parseResponse = (content) => {
  ...
  return parsed.questions
    ...
    .filter(q => {
      ...
      // Verify answer: check if solution text contains simple arithmetic errors
      if (q.answerFormat === 'number') {
        const answerNum = parseFloat(q.answer);
        if (!isNaN(answerNum)) {
          const solText = (q.solutionBlocks || []).map(b => b.value || '').join('');
          // Only check unambiguous operators: + - × ÷ (skip / and * which conflict with fractions)
          ...
          if (calcPatterns) {
            for (const p of calcPatterns) {
              ...
              if (m) {
                ...
                else if (op === '÷') expected = b !== 0 ? a / b : null;
...
exports.main = async (event, context) => {
  ...
  try {
    ...
    const maxTokens = count === 1 ? 1200 : 3000;
